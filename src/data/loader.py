...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if lang in params.ae_steps or lang in params.bt_src_langs or (lang, None) in params.mass_steps:
...
def load_para_data(params, data):
    ...
    required_para_train = required_para_train | set([(l1, l3) for l1, _, l3 in params.xbt_steps])
...
def check_data_params(params):
    ...
    mass_steps = [s.split('-') for s in params.mass_steps.split(',') if len(s) > 0]
    params.mass_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in mass_steps]
    ...
    required_mono = set([l1 for l1, l2 in (params.mlm_steps + params.clm_steps + params.mass_steps) if l2 is None] + params.ae_steps + params.bt_src_langs)
    params.mono_dataset = {
        lang: {
    ...
    required_para_train = required_para_train | set([(l1, l3) for l1, _, l3 in params.xbt_steps])
    required_para_train = required_para_train | set([(l1, l3) for l1, _, l3 in params.rat_steps])
    required_para_train = required_para_train | set([(l1, l3) for l1, _, l3 in params.rabt_steps])
    ...
    assert params.eval_bleu is False or len(params.mt_steps + params.bt_steps + params.xbt_steps + mass_steps) > 0
