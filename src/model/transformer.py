...
class PredLayer(nn.Module):
    ...
    def forward(self, x, y, get_scores=False, smoothing=False):
        """
        ...
        if self.asm is False:
            if smoothing and hasattr(self, 'smoothing'):
                scores = self.proj(x).view(-1, self.n_words)
                loss = self.smoothie(scores, y)                
            else:
                scores = self.proj(x).view(-1, self.n_words)
...
class TransformerModel(nn.Module):
    ...
    def tie_lang_embs(self, lang1, lang2):
        # avg = (self.lang_embeddings.weight[lang1] + self.lang_embeddings.weight[lang2]) / 2
        # self.lang_embeddings.weight[lang1] = avg.clone()
        # self.lang_embeddings.weight[lang2] = avg.clone()
        with torch.no_grad():
            self.lang_embeddings.weight[lang1] = self.lang_embeddings.weight[lang2]
    ...
    def predict(self, tensor, pred_mask, y, get_scores, smoothing=0.0):
        """
    ...
    def ra_generate(self, src_enc, src_len, ref_enc, ref_len, tgt_lang_id, max_len=200, sample_temperature=None):
        """
        Decode a sentence given initial start.
        `x`:
            - LongTensor(bs, slen)
        ...
        while cur_len < max_len:
            ...
            tensor1 = self.forward(
                ...
                src_enc=src_enc,
                src_len=src_len,
                cache=None # NOTE: can't fit both caches on V100 gpu
            )
            ...
            # print(tensor1.size())
            tensor1 = tensor1.data[-1, :, :]               # (bs, dim)
            tensor2 = tensor2.data[-1, :, :]               # (bs, dim)
