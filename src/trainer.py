...
class EncDecTrainer(Trainer):
    ...
    def rat_step(self, src_lang, tgt_lang, ref_lang, back, lambda_coeff):
            """
            Ref-agreement-translation step for machine translation.
            """
            assert lambda_coeff >= 0
            if lambda_coeff == 0:
                return
            assert src_lang != tgt_lang != ref_lang
            params = self.params
            _encoder = self.encoder.module if params.multi_gpu else self.encoder
            _decoder = self.decoder.module if params.multi_gpu else self.decoder
            ...
            ref_lang_id = params.lang2id[ref_lang]

            # generate source batch
            (src_x, src_len), (ref_x, ref_len) = self.get_batch('rat', src_lang, ref_lang)

            src_langs = src_x.clone().fill_(src_lang_id)
            ref_langs = ref_x.clone().fill_(ref_lang_id)
            # cuda
            ...
            if not back:
                ...
                pred_mask = alen[:, None] < tgt_len[None] - 1  # do not predict anything given the last target word
                tgt_y = tgt_x[1:].masked_select(pred_mask[:-1])
            ...
            if back:
                ...
                # decode decode to src
                dec_tgt_src = self.decoder('fwd', x=src_x, lengths=src_len, langs=src_langs, causal=True, src_enc=tgt_enc, src_len=tgt_len)                
            ...
            if back:
                ...
                self.stats[('RAT-%s-%s-%s' % (src_lang, tgt_lang, ref_lang))].append(loss.item())
            ...
            # number of processed sentences / words
            self.n_sentences += params.batch_size
            self.stats['processed_s'] += src_len.size(0)
            self.stats['processed_w'] += (src_len - 1).sum().item()
    ...
    def mass_step(self, lang1, lang2, lambda_coeff):
        ...
        x_, len_ = self.get_batch('mass', lang1)
        (x1, len1, x2, len2, y, pred_mask, positions) = self.restricted_mask_sent(x_, len_, int(params.lambda_span))

        if lang1 != lang2:
            # x2_, len2_, positions, langs, _ = self.generate_batch(lang1, lang2, 'mass')
            x2_, len2_ = self.get_batch('mass', lang2)
            (x1, len1, x2, len2, y, pred_mask, positions) = self.restricted_mask_sent(x_, len_, int(params.lambda_span))
            (x3, len3, x4, len4, y2, pred_mask2, positions2) = self.restricted_mask_sent(x2_, len2_, int(params.lambda_span))

            x1, len1, positions, langs1 = concat_batches(x1, len1, lang1_id, x3, len3, lang2_id, params.pad_index, params.eos_index, reset_positions=True)
            print(y.size(), y)
            print(y2.size(), y2)
            print(pred_mask.size(), pred_mask)
            print(pred_mask2.size(), pred_mask2)
            # x2, len2, _, _ = concat_batches(x2, len2, lang1_id, x4, len4, lang2_id, params.pad_index, params.eos_index, reset_positions=True)
        ...
        _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False)
        if lang1 == lang2:
            self.stats[('MA-%s' % lang1)].append(loss.item())
        else:
